...
CREATE TABLE locations (
    ...
    phone_number TEXT CHECK(LENGTH(phone_number) BETWEEN 8 AND 20),
    email TEXT UNIQUE CHECK(email LIKE '%_@_%._%'), -- Ensure emails have '@', '.' and at least a character before and after each one to be valid
    opening_hours TEXT CHECK(LENGTH(opening_hours) BETWEEN 9 AND 11)
);
...
CREATE TABLE members (
    ...
    date_of_birth DATE,
    join_date TEXT DEFAULT (DATE('now')), -- To allow DATE('now') to be used as a default value, use TEXT instead of DATE
...
CREATE TABLE staff (
    ...
    FOREIGN KEY (location_id) REFERENCES locations(location_id) -- Each staff member must be linked to a valid location
);
...
-- Inserting values into the tables manually
...
-- SELECT * FROM member_health_metrics;
-- SELECT * FROM equipment_maintenance_log;